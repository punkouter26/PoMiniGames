...
- **PoSurvive inference has two backends.** `Inference:UseMock` defaults to **true in code**, but `wwwroot/appsettings.json` ships **false**, so the real default is the live path: `InferenceBootstrapper` prefers the server relay (no download) and only falls back to in-browser WebLLM when the relay is unavailable. Before starting that download it now calls `gpuProbe.checkGpuLabel` — the probe script existed for months with no C# caller, so the boot screen's "GPU" line was a caption and a browser with no hardware WebGPU adapter pulled several hundred MB it could not use. No adapter now means an instant scripted fallback instead. Server-side, `Inference:UseCloudFallback` gates whether `POST /api/infer` is mapped at all, and `Inference:RemoteModelOptions` is an allowlist of model ids a client may name, mapped to real deployments. `/api/infer/status` reports the truth either way. If the PoSurvive AI appears to do nothing, check these before assuming a bug.
