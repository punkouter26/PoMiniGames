...
- **100/50/25/25 rule**: *every* tier has its own ceiling, each enforced by its own guard test — Unit 100 (`TestCountCeilingTests`), Integration 50 (`IntegrationTestCountCeilingTests`), E2E-API 25, E2E-UI 25. They count `[Fact]`/`[Theory]` **methods**, not discovered cases. When one trips, consolidate or relocate — never raise the cap. Unit is at/near its ceiling, so route new tests to Integration by default, but note Integration has only 50 slots and is not a dumping ground.
...
- **Stale docs — do not trust, do not "fix" unasked.** `README.md` is badly out of date (wrong game names, "Radzen UI", a `src/PoMiniGames/` + `PoShared/` layout that no longer exists). `scripts/README.md` documents a `poshared-dryrun.ps1` that isn't in the repo and omits ~45 scripts that are. Trust `src/PoMiniGames.Client/Games/` and `src/PoMiniGames.API/Features/` over any table in either file. `scripts/` has also accumulated one-off debug JS (`find-real-bad2.js`, `_tmp2.mjs`, `try-import.js`) that belongs to no workflow — `test-all.ps1`, `setup.ps1`, `smoke-local.ps1`, and `deploy-preflight.ps1` are the live ones.
