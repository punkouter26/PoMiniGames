...
export const PICK = Object.freeze({ maxDistance: 60, radiusPadding: 0.35, touchPadding: 1.2 });
 ...
 */
export function pickCreature(origin, dir, view, handles, count, { padding = PICK.radiusPadding, maxDistance = PICK.maxDistance } = {}) {
  let best = null;
  for (let k = 0; k < count; k++) {
    const o = k * FRAME.CREATURE_STRIDE;
    const sp = SPECIES[view[o + 5] | 0];
    const scale = (view[o + 4] || 1) * (view[o + 7] === 0 ? 0.55 : 1);
    const r = (sp?.radius ?? 0.4) * scale + padding;
